...
[![살도 로그인 화면](docs/images/login.png)](https://saldobook.coders.kr)

실제 배포된 공개 로그인 화면입니다. Google 또는 카카오의 공식 인증 화면을 거쳐 로그인하며, 소셜 계정 비밀번호는 살도에 전달되거나 저장되지 않습니다. 이미지를 누르면 운영 서비스로 이동합니다.

로그인 후 대시보드는 개인 금융정보가 표시되므로 공개 저장소에는 캡처를 올리지 않습니다. 추가 스크린샷을 등록할 때는 사용자 이름, 개인 ID, 계좌번호, 잔액과 거래내역을 반드시 마스킹해야 합니다.
